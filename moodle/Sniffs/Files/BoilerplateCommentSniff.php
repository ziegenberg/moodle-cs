...
class BoilerplateCommentSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr): void
    {
        ...
        //
        // In PHPCS 4 the T_OPEN_TAG token no longer includes the trailing whitespace (newline),
        // so we cannot rely on the comment being at $expectedafter + 1. Instead we verify that
        // the first comment starts on the line immediately following the opening tag/annotation.
        if ($tokens[$firstcommentptr]['line'] !== ($tokens[$expectedafter]['line'] + 1)) {
            ...
            // In PHPCS 4 the whitespace following the T_OPEN_TAG is tokenized separately and
            // may still carry the opening tag's line number, so look for a token on the next line.
    ...
    private function insertBoilerplate(File $file, int $stackptr): void
    {
        ...
        if ($tokens[$stackptr]['code'] === T_OPEN_TAG) {
            // In PHPCS 4 the T_OPEN_TAG token no longer contains a trailing newline.
            // Remove any whitespace (on the same line) following the open tag so that the
            // boilerplate starts on the expected line and we don't end up with a double newline.
    ...
    private function moveBoilerplate(File $file, int $start, int $target): void
    {
        ...
        if ($nextnonwhitespace === false || $nextnonwhitespace === $target) {
            ...
            // In PHPCS 4 the T_OPEN_TAG token no longer contains a trailing newline,
            // so add it back after removing the (now redundant) whitespace tokens.
